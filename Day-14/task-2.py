...
def dropSand(biggestY):
	...
	while currentY <= biggestY:
		...
		break
	# We now return the new coords the sand has moved to
	return (currentX, currentY)
...
def main():
	...
	while True:
		x, y = dropSand(biggestY)
		rocks.add((x, y))
		total += 1
		# If the sand has reached the drop point, no more sand can drop
		if (x, y) == (500, 0):
			break
